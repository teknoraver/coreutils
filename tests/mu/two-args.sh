#!/bin/sh
# Make sure 'mu d/1 d/2' works.
# That command failed with mu from fileutils-4.0q.
...
# so that the "mu .." below doesn't traverse sibling directories
...
test -d $t || fail=1
mu $t/1 $t/2 > /dev/null || fail=1

# Make sure 'mu . $t' and 'mu .. $t' work.
# These would fail prior to fileutils-4.0y.
mu . $t > /dev/null || fail=1
mu .. $t > /dev/null || fail=1
